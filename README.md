...
for n = 0:N
    if n == N/2 then
        ...
        h(n+1) = sin(2*%pi*fc/Fs*(n-N/2)) / (%pi*(n-N/2));
    end
end
...
// Insert zeros
x_int = zeros(1,length(x_dec)*L);

x_int(1:L:$) = x_dec;

// Low-pass filter after interpolation
fc2 = Fs/(2*L);
...
for n = 0:N
    if n == N/2 then
        ...
        h2(n+1) = sin(2*%pi*fc2/Fs*(n-N/2)) / (%pi*(n-N/2));
    end
end

// Apply interpolation filter
x_interp = convol(x_int,h2);
...
subplot(3,1,2);
plot(x_dec);
xtitle("Decimated Signal (M = 2)","Samples","Amplitude");

// Interpolated signal
subplot(3,1,3);
plot(x_interp);
xtitle("Interpolated Signal (L = 2)","Samples","Amplitude");

# OUTPUT and spectrum for Decimation and Interpolation
<img width="1320" height="707" alt="image" src="https://github.com/user-attachments/assets/bb9464cb-cb42-4646-81e4-c7913ee01538" />
